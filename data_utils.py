...
class ABSAProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, task_type, fn=".train.txt"):
        ...
        return self._create_examples(
            ...
            # self.read_txt(os.path.join('./under_datasets', source + fn)), task_type=task_type, set_type="train")   # 其他欠采样
            # self.read_txt(os.path.join('./ping_datasets', source + fn)), task_type=task_type, set_type="train")
            # self.read_txt(os.path.join('./yuan_datasets', source + fn)), task_type = task_type, set_type = "train")   # 随机欠采样
    ...
    def get_target_examples(self, data_dir, task_type, fn=".txt"):
        """See base class."""
        source, target = data_dir.split('-')
        target = 'new_' + target
        return self._create_examples(
            self.read_txt(os.path.join('./MMD_data', target + fn)), task_type= task_type, set_type="test")
            # self.read_txt(os.path.join('./run_out/get_filter/' + data_dir, target + fn)), task_type= task_type, set_type="test")
...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs=1):
        ...
        with open(data_file, 'r') as f:   # 添加一个encoding
