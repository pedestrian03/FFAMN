...
def evaluate_chunk(test_Y, pred_Y):
    ...
    return precision, recall, F1,TP+FP,TP
...
def evaluate_absa_chunk(test_Y, pred_Y):
    ...
    return precision_a, recall_a, F1_a,TP_A+FP_A,TP_A
...
def eval_ts_result(dir):
    ...
    with open(input_file, 'r') as fp:  # 去掉encoding='utf-8'
        lines = fp.readlines()
    dataset = []
    test_Y, pred_Y = [], []
    for l in lines:
    ...
    ad_p, ad_r, ad_f1, ad_pred_len, ad_right_len = evaluate_chunk(test_Y, pred_Y)
    ads_p, ads_r, ads_f1, absa_pred_len, absa_right_len = evaluate_absa_chunk(test_Y, pred_Y)
    print('Aspect extract result: precision: {} recall: {} f1: {}'.format(ad_p, ad_r, ad_f1))
    print('pred_len: {} right_len: {}'.format(ad_pred_len, ad_right_len))
    print('Main results for End2end ABSA task： precision: {} recall: {} f1: {}'.format(ads_p, ads_r, ads_f1))
    print('pred_len: {} right_len: {}'.format(absa_pred_len, absa_right_len))
...
if __name__ == '__main__':
    tag_sequence = ['I-Aspect', 'I-Aspect', 'O', 'B-Aspect', 'O', 'B-Opinion', 'I-Opinion']
    print(tag2aspect(tag_sequence))  # 函数总会从一个B标签开始提取